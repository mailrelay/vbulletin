...
class mrsyncController 
{
     ...
     * @return curl
     */
    public function initCurl( $hostname = '')
    {
    ...
    /**
     * Validate user against MR API and return API key
     * 
     * @param string $username MR username asigned
     * @param string $password MR password asigned
    ...
    {            
        ...
        if (!$jsonResult->status) {
            $warning = $this->_vbphrase['api_key_fail'];
            exec_header_redirect('mrsync_admin.php?do=showLogin&message=' . $warning);
        } else {
     ...
     * 
     * @param curl $curl
     * @param string $apiKey apiKey obtained using getApiKey method
     ...
     * 
     */
    public function showGroupsForm( $hostname = '', $username = '', $password = '', $groupSelect = array(), $vBulletinGroups = array(), $vBulletinSocialGroups = array() )
    {
        if ( !empty($hostname) && !empty($username) && !empty($password) && (count($groupSelect) > 0) ) {
     ...
     */
    public function checkRequiredFormValues( $hostname = '', $username = '', $password = '' )
    {
