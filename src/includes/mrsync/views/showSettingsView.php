...
// Mailrelay account Hostname, key
...
print_input_row('<strong>' . $this->_vbphrase['sync_api_key'] . '</strong>', 'key', $formValues['key']);
...
if (!empty($formValues['hostname']) && !empty($formValues['key']) ) {
...
print_input_row('<strong>' . $this->_vbphrase['mrsync_smtp_user'] . '</strong>', 'smtpUser', $formValues['smtpUser']);
print_input_row('<strong>' . $this->_vbphrase['mrsync_smtp_pass'] . '</strong>', 'smtpPass', $formValues['smtpPass']);
print_description_row('<br/><br/>', false, 2);
